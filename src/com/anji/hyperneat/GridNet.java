...
public class GridNet implements Activator {
    ...
    private float[][][][][][] weights; //z1,y1,x1,z2,y2,x2
    
    private float[][][] bias; //z,y,x

    private int depth;
    private int[] width; //dimensions for each layer
    private int[] height;
    
    private float[][][] activation; //tz,ty,tx
    private float[][][] activationNew; //tz,ty,tx
     ...
     * @param connectionMaxRanges The maximum length/range of connections in
     * either direction in the x, y and z axes. The array has format
     * [z=0|y=1|x=2],[negative=0|positive=1] = max range.
     * For example a standard feed-forward network with width W
     * (x-axis), height H (y-axis) and depth D (z-axis) would be described:
     * {{-1, 1}, {H, H}, {W, W}}, where -1 for the negative direction for the
     * z-axis indicates that connections may not go from layer n to layer &lt; n.
     * @param weights The connection weights of the network between each neuron,
     * where neurons are specfied by their coordinates, in the format
     ...
     * x1, y1, z1 is the target and xR, yR, zR is the source. Note that the
     * range of connections for neurons at or close to the edge of the grid must
     * be circumscribed by the edge of the grid, so the last three dimensions
     * of the array must have differing sizes to account for this (except in the
     * unlikely scenario that connections extend only straight down); also note
     * that no connections may extend into the input layer so the connection
     * matrices must also be circumscribed accordingly for this (thus the number of
     * layers in the network including the input layer is weights.length+1).
     * @param bias the bias for each neuron not in the input layer, in the format 
     * [z][y][x] where z, y, and x are the coordinates of the neuron (as input 
     * neurons don't receive a bias bias.length should match weights.length).
     * @param function the ActivationFunction to use; only one type of
     ...
     */
    public GridNet(int[][][] connectionMaxRanges, int[][] layerDimensions, float[][][][][][] weights, float[][][] bias, ActivationFunction function, int cyclesPerStep, String aName) {
        ...
        activation = new float[depth][][];
        activationNew = new float[depth][][];
        for (int l = 0; l < depth; l++) {
        	activation[l] = new float[height[l]][width[l]];
        	activationNew[l] = new float[height[l]][width[l]];
        }
    ...
    /**
     * Creates a GridNet with the given specifications.
     * @param connectionMaxRange The maximum length/range of connections in
     * either direction in the x, y and z axes.
     * @param weights The connection weights of the network between each neuron,
     * where neurons are specfied by their coordinates, in the format
     * [z1][y1][x1][zR][yR][xR], where xR, yR, and zR are relative to x1, y1
     * and z1, and with the corresponding negative direction ranges from
     ...
     * @param aName Name of the network.
     */
    public GridNet(int connectionMaxRange, int[][] layerDimensions, float[][][][][][] weights, float[][][] bias, ActivationFunction function, int cyclesPerStep, String aName) {
        depth =  weights.length + 1;
        width = layerDimensions[0];
        height = layerDimensions[1];
    ...
    /**
     * Creates a feed-forward GridNet with the given
     * specifications.
     * @param connectionMaxRange The maximum length/range of connections in
     * either direction in the tx and ty axes.
     * @param weights The connection weights of the network between layers.
     * Format is [l-1][y1][x1][yR][xR], where x1 and y1 are the coordinates of the
     * target neuron in layer l and xR and yR are the relative (to x1 and y1)
     * coordinates of the source neuron in layer l-1,
     * and with the corresponding negative direction ranges from
     * connectionMaxRanges added to avoid negative array indices. Note that the
     * range of connections for neurons at or close to the edge of the grid must
     * be circumscribed by the edge of the grid, so the last two dimensions
     * of the array must have differing sizes to account for this (except in the
     * unlikely scenario that connections extend only straight down). The number 
     * of layers in the network including the input layer is weights.length+1.
     * @param bias the bias for each neuron not in the input layer, in the format 
     * [z][y][x] where z, y, and x are the coordinates of the neuron (as input 
     * neurons don't receive a bias bias.length should match weights.length).
     * @param function the ActivationFunction to use; only one type of
     ...
     * @param aName Name of the network.
     */
    public GridNet(int connectionMaxRange, int[][] layerDimensions, float[][][][][] weights, float[][][] bias, ActivationFunction function, String aName) {
        depth =  weights.length + 1;
        width = layerDimensions[0];
        height = layerDimensions[1];
     ...
     * specifications.
     * @param connectionMaxRanges The maximum length/range of connections in
     * either direction in the tx and ty axes. The array has format
     ...
     * @param aName Name of the network.
     */
    public GridNet(int[][][] connectionMaxRanges, int[][] layerDimensions, float[][][][][] weights, float[][][] bias, ActivationFunction function, String aName) {
        depth =  weights.length + 1;
        width = layerDimensions[0];
        height = layerDimensions[1];
    ...
    /**
     * Creates a fully-connected feed-forward GridNet with the given
     * specifications.
     * @param weights The connection weights of the network between layers.
     * Format is [l-1][y1][x1][y2][x2], where x1 and y1 are coordinates of
     * target neuron in layer l and x2 and y2 are coordinates of source neuron
     * in layer l-1. The number 
     * of layers in the network including the input layer is weights.length+1.
     * @param bias the bias for each neuron not in the input layer, in the format 
     * [z][y][x] where z, y, and x are the coordinates of the neuron (as input 
     * neurons don't receive a bias bias.length should match weights.
     ...
     * @param aName Name of the network.
     */
    public GridNet(float[][][][][] weights, int[][] layerDimensions, float[][][] bias, ActivationFunction function, String aName) {
        depth =  weights.length + 1;
        width = layerDimensions[0];
        height = layerDimensions[1];
    ...
    private void initFeedForward(int[][][] connectionMaxRanges, float[][][][][] weights, ActivationFunction function, String aName) {
        //System.out.println("GridNet dimensions (DxHxW): " + depth + " x " + height + " x " + width);
        ...
        float[][][][][][] w = new float[depth-1][][][][][];
        //for each neuron zyx
        for (int z = 0; z < depth-1; z++) {
        	w = new float[z][height[z+1]][width[z+1]][1][][];
    ...
    /**
     * Perform one activation step, consisting of cyclesPerStep steps,
     * calculating new activation for all neurons.
     * For feed-forward networks use stepFF.
     */
    public void step() {
            ...
            for (int tz = 1; tz < depth; tz++) { //first layer is input layer, don't update it
                for (int ty = 0; ty < height[tz]; ty++) {
                    for (int tx = 0; tx < width[tz]; tx++) {
                        ...
                        for (int wz = 0, sz = Math.max(0, tz + wz - connectionMaxRanges[tz-1][0][0]);
                        wz < w.length;
                        wz++, sz++) {

                            for (int wy = 0, sy = Math.max(0, ty + wy - connectionMaxRanges[tz-1][1][0]);
                            wy < w[wz].length;
                            wy++, sy++) {

                                for (int wx = 0, sx = Math.max(0, tx + wx - connectionMaxRanges[tz-1][2][0]);
                                wx < w[wz][wy].length;
                                wx++, sx++) {
        ...
        for (int tz = 1, sz = 0; tz < depth; tz++, sz++) { //first layer is input layer, don't update it
            for (int ty = 0; ty < height[tz]; ty++) {
                for (int tx = 0; tx < width[tz]; tx++) {
                    ...
                    float[][] w = weights[tz-1][ty][tx][0]; //weight matrix for connections from s{z,y,x} to t{z,y,x}
                    float sum = bias[tz-1][ty][tx];
    ...
    /**
     * @return float[][] output layer given last provided input activation via
     * <code>next(float[])</code> or <code>next(float[][])</code>, copied by
     * reference. For recurrent networks it's probably not a good idea to modify
     * the returned array.
     * @see Activator#next(float[])
     * @see Activator#next(float[][])
     */
    public Object next() {
        if (isFeedForward)
    ...
    /**
     * 
     * @param stimuli first row of input layer. The stimuli array
     * is copied by reference, replacing the original first layer row array.
     * Changes to stimuli after this call outside of this network will
     * be reflected in the network. The network never changes the input layer.
     * @return float[] first row of output array given input stimuli, copied by
     * reference. For recurrent networks it's probably not a good idea to modify the
     * returned array.
     */
    ...
    /**
     * @param stimuli sequence of first row of input layer. The stimuli arrays
     * are copied by reference, replacing the original first layer array.
     * Changes to the last stimuli array after this call outside of this network
     * will be reflected in the network. The GridNet never changes the input
     * layer.
     * @return float[][] sequence of first row of output values array given
     * input stimuli.
     */
    public float[][] nextSequence( float[][] stimuli ) {
        float[][] response = new float[stimuli.length][width[depth-1]];
    ...
    /**
     * @param stimuli input layer. The stimuli array
     * is copied by reference, replacing the original first layer array.
     * Changes to stimuli after this call outside of this network will
     * be reflected in the network. The network never changes the input layer.
     * @return float[][] output layer given input stimuli, copied by reference.
     */
    public float[][] next( float[][] stimuli ) {
    ...
    /**
     * @param stimuli sequence of input layer values. The stimuli arrays
     * are copied by reference, replacing the original first layer array.
     * Changes to the last stimuli array after this call outside of this network
     * will be reflected in the network. The network never changes the values
     * in the input layer.
     * 
     * @return float[][][] sequence of output value arrays given input
     * stimuli.
     */
    public float[][][] nextSequence( float[][][] stimuli ) {
        float[][][] response = new float[stimuli.length][height[depth-1]][width[depth-1]];
        for (int seq = 0; seq < stimuli.length; seq++) {
    ...
    */
    ...
    public static void main(String[] args) {
        /*
        float[][][] array = new float[5][500][500];
        //float[][][] arrayEmpty = new float[5*500*500];
        ...
        for (int i = 0; i < 2500; i++) {
            ...
            for (int tx = 0; tx < 5; tx++) {
                for (int ty = 0; ty < 500; ty++) {
                    for (int tz = 0; tz < 500; tz++) {
                        array[tx][ty][tz] = 0;
                    }
                    //Arrays.fill(array[tx][ty], 0);
                    //System.arraycopy(arrayEmpty, 0, array[tx][ty], 0, arrayEmpty.length);
                }
            }
...
/*
        int[][] cmr = new int[3][2];
        cmr[0][0] = 1; //tz neg, no connections to previous or own layer
        cmr[0][1] = 1; //tz pos
        cmr[1][0] = 1; //ty neg
        cmr[1][1] = 1; //ty pos
        cmr[2][0] = 1; //tx neg
...
*/
        ...
        int cmr = 2;
/*
        float[][][][][] weights = new float[depth-1][height][width][][];
        //for each target neuron
        for (int z = 0; z < depth-1; z++) {
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    //calculate dimensions of this weight matrix (bounded by grid edges)
                    //int dy = Math.min(height-1, ty+cmr[1][1]) - Math.max(0, ty-cmr[1][0]) + 1;
                    //int dx = Math.min(height-1, tx+cmr[2][1]) - Math.max(0, tx-cmr[2][0]) + 1;
                    int dy = Math.min(height-1, y+cmr) - Math.max(0, y-cmr) + 1;
                    int dx = Math.min(height-1, x+cmr) - Math.max(0, x-cmr) + 1;

                    //System.out.println(tz + "," + ty + "," + tx + "  dy = " + dy + "  dx = " + dx);

                    weights[z][y][x] = new float[dy][dx];
                    float[][] w = weights[z][y][x];

                    //for each connection to zyx
                    //w{y,x} is index into weight matrix
                    //s{y,x} is index of source neuron
                    //for (int wy = 0, ty = Math.max(0, ty-cmr[1][0]);
                    for (int wy = 0, sy = Math.max(0, y-cmr);
                    wy < dy;
                    wy++, sy++) {
                        //for (int wx = 0, tx = Math.max(0, tx-cmr[2][0]);
                        for (int wx = 0, sx = Math.max(0, x-cmr);
                        wx < dx;
                        wx++, sx++) {
        ...
        */
        ...
        float[][][][][][] weights = new float[depth-1][height][width][][][];
        float[][][] bias = new float[depth-1][height][width];
        ...
        for (int tz = 1; tz < depth; tz++) {
            for (int ty = 0; ty < height; ty++) {
                for (int tx = 0; tx < width; tx++) {
                    ...
                    for (int wz = 0, sz = Math.max(0, tz-cmr);
                    wz < dz;
                    wz++, sz++) {
                        for (int wy = 0, sy = Math.max(0, ty-cmr);
                        wy < dy;
                        wy++, sy++) {
                            for (int wx = 0, sx = Math.max(0, tx-cmr);
                            wx < dx;
                            wx++, sx++) {
        ...
        /*
        float[][][][][] weights = new float[depth-1][height][width][height][width];

        for (int tz = 0; tz < depth-1; tz++) {
            for (int ty = 0; ty < height; ty++) {
                for (int tx = 0; tx < width; tx++) {
                    //for each connection from zyx
                    for (int wy = 0; wy < height; wy++) {
                        for (int wx = 0; wx < width; wx++) {
                            if (Math.abs(ty-wy) <= 1 && Math.abs(tx-wx) <= 1)
                                weights[tz][ty][tx][wy][wx] = 0.5/(0.5+Math.sqrt((ty-wy)*(ty-wy) + (tx-wx)*(tx-wx)));
                            else
                                weights[tz][ty][tx][wy][wx] = 0;
                        }
                    }
                }
            }
...
/*
        ...
        float[][] input = net.getInputs();

        for (int ty = 0, v = 1; ty < input.length; ty++) {
            for (int tx = 0; tx < input[0].length; tx++, v++) {
                input[ty][tx] = v;
                //input[ty][tx] = 1;
            }
        }
