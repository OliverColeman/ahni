...
/**
 * Converts run data to XML presentation format. Complexity, fitness, and speciation are represented with the same XML
 * structure, different stylesheets. A new <code>XmlPersistableRun</code> object should be created new for each use,
 * because it caches the XML it produces, and does not automatically detect changes to the underlying <code>Run</code>
 * object.
 * 
...
public class XmlPersistableRun implements XmlPersistable {
	...
	public XmlPersistableRun(Run aRun) {
		...
		if (props != null) {
			...
			params.append("<population-size>").append(props.getIntProperty(NeatConfiguration.POPUL_SIZE_KEY)).append("</population-size>\n");
			params.append("<generations>").append(props.getIntProperty(Evolver.NUM_GENERATIONS_KEY)).append("</generations>\n");
			params.append("</search-parameters>\n");
		}
	...
	protected void appendToString(boolean includeDtd, StringBuffer result) {
		if (cachedRunXml == null) {
			...
			String startTag = "<" + RUN_TAG + " name=\"" + run.getName() + "\" timedatestamp=\"[" + fmt.format(run.getStartTime().getTime()) + " - " + fmt.format(Calendar.getInstance().getTime()) + "]\" >\n";
