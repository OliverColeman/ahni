...
/**
 * A Gene implementation that supports a integer values for its allele. Upper and lower bounds may optionally be
...
public class IntegerAllele extends Allele {
	...
	/**
	 * Represents the delimiter that is used to separate fields in the persistent representation of IntegerGene
	 * instances.
	 */
	...
	/**
	 * Stores the number of integer range units that a single bounds-range unit represents. For example, if the integer
	 * range is -2 billion to +2 billion and the bounds range is -1 billion to +1 billion, then each unit in the bounds
	 * range would map to 2 units in the integer range. The value of this variable would therefore be 2. This mapping
	 * unit is used to map illegal allele values that are outside of the bounds to legal allele values that are within
	 * the bounds.
	 */
	...
	/**
	 * Constructs a new IntegerGene with the given active configuration and the specified lower and upper bounds for
	 * values represented by this Gene.
	 * 
	 * @param a_activeConfiguration The current active configuration.
	...
	public IntegerAllele(Configuration a_activeConfiguration, int a_lowerBounds, int a_upperBounds) {
		super(new Gene((a_activeConfiguration == null) ? new Long(0) : a_activeConfiguration.nextInnovationId()));
	...
	/**
	 * Sets the value (allele) of this Gene to the new given value. This class expects the value to be an Integer
	 * instance. If the value is above or below the upper or lower bounds, it will be mappped to within the allowable
	 * range.
	 * 
	...
	public String getPersistentRepresentation() throws UnsupportedOperationException {
		...
		// --------------------------------------------------------------
		return toString() + PERSISTENT_FIELD_DELIMITER + m_lowerBounds + PERSISTENT_FIELD_DELIMITER + m_upperBounds;
	}
